...
// + the theme.json it references — the same lookup DMS itself performs, and
// the same one djobs already does (kept in sync so both tools' chrome
// matches). Only understands DMS's Catppuccin registry theme; any other
// theme category, registry theme, or missing/malformed file returns "" so
